...
async def update_beacons() -> None:
    """Периодический пересчёт топлива и прочности."""
...
@tasks.loop(minutes=1)
async def check_beacons(bot: BeaconBot) -> None:
    """Проверка уничтожения и низких показателей."""
    try:
        beacons = db.fetch_all_for_update()
        ...
        error_msg = f"Ошибка в check_beacons: {exc}"
...
def start_background_tasks(bot: BeaconBot) -> None:
    """Запускает фоновые циклы (идемпотентно)."""
    if not update_beacons.is_running():
        update_beacons.start()

    if not check_beacons.is_running():
        # tasks.loop с аргументом: передаём bot через before_loop / start kwargs
        check_beacons.start(bot)
